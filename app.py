...
# Ensure Poppler and Tesseract are on PATH (Windows installs)
for _p in [r"C:\poppler\Library\bin", r"C:\Program Files\Tesseract-OCR"]:
...
def _init_db():
    ...
    try:
        ...
        # Sprint 2: Users table migration ? must_reset column
        try:
        ...
        for username, display_name, role in seed_users:
            conn.execute(
                """INSERT OR IGNORE INTO users
                   (username, display_name, role, pin_hash, must_reset, is_active, created_at, updated_at)
                   VALUES (?, ?, ?, ?, 1, 1, ?, ?)""",
    ...
    try:
        row = conn.execute(
            """SELECT id, username, display_name, role, pin_hash,
                      must_reset, is_active, last_login, created_at, updated_at
...
def set_user_pin_hash(user_id, pin_hash, must_reset=None):
    """Set the pin_hash for a user. Optionally update must_reset."""
    conn = _get_db()
    try:
        if must_reset is None:
            conn.execute(
            ...
            conn.execute(
                "UPDATE users SET pin_hash = ?, must_reset = ?, updated_at = ? WHERE id = ?",
                (pin_hash, 1 if must_reset else 0, datetime.now().isoformat(), user_id)
            )
        conn.commit()
    finally:
        conn.close()
...
def create_user(username, display_name, role, pin_hash, must_reset=True):
    ...
    try:
        ...
        cursor = conn.execute(
            """INSERT INTO users
               (username, display_name, role, pin_hash, must_reset, is_active, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, 1, ?, ?)""",
            (username, display_name, role, pin_hash, 1 if must_reset else 0, now, now)
        )
        conn.commit()
...
def build_dashboard_data():
    """Build comprehensive dashboard payload from jobs dict + SQLite tables."""
    import shutil as _shutil
    now = datetime.now()
    today_str = now.strftime("%Y-%m-%d")
    ...
    daily_jobs = {}
    daily_cost = {}
    stage_counts = {"draft": 0, "preparer_review": 0, "reviewer_review": 0,
                    "partner_review": 0, "final": 0}
    client_doc_counts = {}
    recent_activity = []
    ...
    for jid, j in jobs.items():
        ...
        if st and et:
            try:
                delta = (datetime.fromisoformat(et) - datetime.fromisoformat(st)).total_seconds()
                if delta > 0:
                    runtimes.append(delta)
            except (ValueError, TypeError):
                pass

        dt = j.get("doc_type", "other") or "other"
        doc_type_counts[dt] = doc_type_counts.get(dt, 0) + 1

        created = j.get("created", "")
        created_date = created[:10] if created else ""
        if created_date >= cutoff_14d:
            daily_jobs[created_date] = daily_jobs.get(created_date, 0) + 1
            daily_cost[created_date] = daily_cost.get(created_date, 0) + (cost if isinstance(cost, (int, float)) else 0)

        stage = j.get("review_stage", "draft")
        if status == "complete" and stage in stage_counts:
            stage_counts[stage] += 1

        cn = j.get("client_name", "")
        if cn:
            client_doc_counts[cn] = client_doc_counts.get(cn, 0) + 1

    success_rate = round((success_count / finished_count * 100), 1) if finished_count > 0 else 100.0
    avg_time = round(sum(runtimes) / len(runtimes), 1) if runtimes else 0

    # Fill daily series for charts (last 14 days, zero-fill)
    daily_series = []
    for i in range(13, -1, -1):
        ...
        daily_series.append({
            ...
            "jobs": daily_jobs.get(d, 0),
            "cost": round(daily_cost.get(d, 0), 4),
        })

    # Confidence breakdown from op_runs table
    conf_high = conf_low = conf_review = 0
    try:
        conn = _get_db()
        try:
            row = conn.execute("""
                SELECT COALESCE(SUM(fields_high_conf), 0),
                       COALESCE(SUM(fields_low_conf), 0),
                       COALESCE(SUM(fields_needs_review), 0)
                FROM op_runs WHERE status = 'complete'
            """).fetchone()
            if row:
                conf_high, conf_low, conf_review = row[0], row[1], row[2]
        finally:
            conn.close()
    except Exception:
        pass
    ...
    sorted_jobs = sorted(jobs.items(), key=lambda kv: kv[1].get("created", ""), reverse=True)[:20]
    for jid, j in sorted_jobs:
        recent_activity.append({
            ...
            "created": j.get("created", ""),
            "review_stage": j.get("review_stage", "draft"),
        })

    # Top clients
    top_clients = sorted(client_doc_counts.items(), key=lambda x: x[1], reverse=True)[:10]
    ...
    error_count_7d = sum(1 for j in jobs.values()
                         if j.get("status") in ("error", "failed")
                         and (j.get("created") or "") >= cutoff_7d)
    total_7d = sum(1 for j in jobs.values()
    ...
    return {
        ...
        "charts": {
            ...
            "confidence": {
                "high": conf_high,
                "low": conf_low,
                "needs_review": conf_review,
                "total": conf_high + conf_low + conf_review,
            },
        },
...
def _aftercare_worker():
    ...
    Drains ALL queued tasks, coalesces by job_id, then processes each batch
    in a single DB transaction. This prevents SQLite write-lock contention
    with the Flask request thread when many field verifications arrive rapidly.
    """
    while _aftercare_running or _aftercare_queue:
        _aftercare_event.wait(timeout=1.0)
        _aftercare_event.clear()
        # Drain all pending tasks and coalesce by job_id
        batch = {}  # job_id → merged task
        while _aftercare_queue:
            try:
                task = _aftercare_queue.popleft()
                jid = task["job_id"]
                if jid in batch:
                    # Merge incoming fields into existing batch
                    batch[jid]["incoming"].update(task.get("incoming", {}))
                    batch[jid]["field_count"] += task.get("field_count", 1)
                    for s, c in task.get("statuses", {}).items():
                        batch[jid]["statuses"][s] = batch[jid]["statuses"].get(s, 0) + c
                else:
                    batch[jid] = task
            except Exception as e:
                print(f"  Aftercare drain error: {e}")
        # Process each coalesced batch
        for jid, merged_task in batch.items():
            try:
                _process_aftercare(merged_task)
            except Exception as e:
                print(f"  Aftercare error: {e}")


def _process_aftercare(task):
    """Run deferred work for a (possibly coalesced) confirm batch.
    ...
    # 1. Canonical promotion + FactStore (single pass for all coalesced fields)
    _aftercare_promote_facts(job_id, incoming)

    # 2. Verify summary (once per batch, not per field)
    try:
    ...
    # 3. Audit log (single entry for batch)
    try:
        if mode == "guided" and field_count == 1:
...
def save_job(job_id):
    """Persist a single job to SQLite. Much faster than save_jobs() for aftercare."""
    j = jobs.get(job_id)
    if not j:
        return
    with _jobs_lock:
        ...
        try:
            ...
            conn.execute(
                """INSERT INTO jobs (id, data, status, client_name, created, updated, review_stage, stage_updated)
                   ...
                   ON CONFLICT(id) DO UPDATE SET
                       ...
                       client_name = excluded.client_name, updated = excluded.updated,
                       review_stage = excluded.review_stage, stage_updated = excluded.stage_updated""",
                (job_id, json.dumps(safe, default=str), j.get("status", ""),
                 j.get("client_name", ""), j.get("created", ""),
                 datetime.now().isoformat(),
                 j.get("review_stage", "draft"),
                 j.get("stage_updated", ""))
            ...
            print(f"  Warning: Could not save job {job_id} to database: {e}")
        finally:
            conn.close()
...
def login_post():
    ...
    log_event("info", "login_success",
              ...
              user_id=u["id"], ip_addr=ip)
    if u.get("must_reset"):
        log_event("warn", "pin_reset_required",
                  f"{u['display_name']} must reset PIN",
                  user_id=u["id"], ip_addr=ip)
        return redirect("/reset-pin", code=303)
...
# ??? Sprint 2: Self-service PIN reset ?????????????????????????????????????????
@app.route("/reset-pin", methods=["GET"])
@require_login
def reset_pin_page():
    u = current_user()
    if not u or not u.get("must_reset"):
        return redirect("/", code=303)
    return render_template("reset_pin.html", current_user=u, error=None)
...
def reset_pin_post():
    ...
    if not pin.isdigit() or len(pin) != 6:
        return render_template("reset_pin.html", current_user=u,
                               error="PIN must be exactly 6 digits")
    if pin != pin2:
        return render_template("reset_pin.html", current_user=u,
                               error="PINs do not match")
    set_user_pin_hash(u["id"], generate_password_hash(pin), must_reset=False)
    log_event("info", "pin_reset",
              f"{u['display_name']} reset PIN",
...
def admin_create_user():
    ...
    new_id = create_user(username, display_name, role, pin_hash, must_reset=True)
...
def admin_reset_pin(user_id):
    temp_pin = generate_6_digit_pin()
    set_user_pin_hash(user_id, generate_password_hash(temp_pin), must_reset=True)
...
@require_login
def upload_multi():
    """Upload one or more PDFs. mode=single merges into one job; mode=separate creates one job per file."""
    ...
    disable_pii = request.form.get("disable_pii") == "true"
    use_ocr_first = request.form.get("use_ocr_first") == "true"
    user_notes = request.form.get("user_notes", "").strip()[:2000]
    ai_instructions = request.form.get("ai_instructions", "").strip()[:2000]
    resolved_client = _safe_client_name(client_name)

    if mode == "single" and len(files) > 1:
        # Merge PDFs into one and create a single job
        try:
            from pypdf import PdfMerger
        except ImportError:
            from PyPDF2 import PdfMerger
        job_id = datetime.now().strftime("%m%d") + "-" + str(uuid.uuid4())[:6]
        merged_path = UPLOAD_DIR / (job_id + ".pdf")
        merger = PdfMerger()
        filenames = []
        for f in files:
            tmp_path = UPLOAD_DIR / (job_id + "_" + f.filename)
            f.save(str(tmp_path))
            merger.append(str(tmp_path))
        ...
        upload_user = current_user()
        log_event("info", "upload_received",
                  f"Multi-upload (merged): {len(filenames)} PDFs for {resolved_client} ({doc_type})",
                  user_id=upload_user["id"] if upload_user else None, job_id=job_id,
                  details={"filenames": filenames, "client": resolved_client, "doc_type": doc_type, "year": year})
        t = threading.Thread(target=run_extraction, args=(job_id, merged_path, year, skip_verify, doc_type, output_format, user_notes, ai_instructions, disable_pii, False, use_ocr_first))
        t.daemon = True
        t.start()
        return jsonify({"job_id": job_id, "job_ids": [job_id]})
    else:
        # Single file — just redirect to normal upload logic
        if len(files) == 1:
            f = files[0]
            job_id = datetime.now().strftime("%m%d") + "-" + str(uuid.uuid4())[:6]
            pdf_path = UPLOAD_DIR / (job_id + ".pdf")
            ...
            client_dir = _client_dir(resolved_client, doc_type, year)
            jobs[job_id] = {
                "id": job_id, "filename": f.filename, "client_name": resolved_client,
                "doc_type": doc_type, "output_format": output_format,
                "user_notes": user_notes, "ai_instructions": ai_instructions,
                "year": year, "status": "queued", "stage": "queued", "progress": 0,
                "log": [], "created": datetime.now().isoformat(),
                "pdf_path": str(pdf_path), "client_folder": str(client_dir),
                "disable_pii": disable_pii, "use_ocr_first": use_ocr_first,
            }
            save_job(job_id)
            t = threading.Thread(target=run_extraction, args=(job_id, pdf_path, year, skip_verify, doc_type, output_format, user_notes, ai_instructions, disable_pii, False, use_ocr_first))
            t.daemon = True
            t.start()
            return jsonify({"job_id": job_id, "job_ids": [job_id]})
        return jsonify({"error": "Separate mode should use /api/upload per file"}), 400
...
def ai_chat(job_id):
    ...
    # Build context from FactStore (DB-grounded facts)
    extraction_context = ""
    facts_context = ""
    try:
        from fact_store import FactStore
        fs = FactStore(str(DB_PATH))
        facts = fs.get_facts_for_job(job_id)
        if page_num:
            facts = [f for f in facts if f.get("source_page") == page_num]
        if facts:
            lines = []
            for f in facts:
                val = f.get("value_num") if f.get("value_num") is not None else f.get("value_text")
                lines.append(f"{f.get('fact_key')}: {val} (status={f.get('status')})")
            facts_context = "Facts from database:\n" + "\n".join(lines[:200])
            if len(lines) > 200:
                facts_context += f"\n...({len(lines) - 200} more facts)"
    except Exception:
        facts_context = ""

    # Fallback: verified extraction summary
    verified_log = get_verified_extractions(job_id)
    if verified_log and not facts_context:
...
def _load_extraction_log(job_id):
    ...
    Cached by (job_id, file mtime) so repeated calls during review skip disk I/O.
    """
    job = jobs.get(job_id)
    if not job:
        return None
    ...
    try:
        ...
        if len(_extraction_log_cache) > 20:
            oldest = next(iter(_extraction_log_cache))
            del _extraction_log_cache[oldest]
        return data
    except (json.JSONDecodeError, IOError):
        return None
...
def _save_verify_combined(job_id, vdata, incoming_fields):
    """Single-connection fast path: save verifications + upsert verified_fields in one transaction.

    Combines _save_verifications(skip_summary=True) + _upsert_verified_fields_fast()
    into a single DB connection to minimize SQLite write-lock hold time.
    """
    vdata["updated"] = datetime.now().isoformat()
    conn = _get_db()
    try:
        # 1. Save verifications blob
        conn.execute(
            """INSERT INTO verifications (job_id, data, updated) VALUES (?, ?, ?)
               ON CONFLICT(job_id) DO UPDATE SET data = excluded.data, updated = excluded.updated""",
            (job_id, json.dumps(vdata, default=str), vdata["updated"])
        )

        # 2. Upsert verified_fields rows
        if incoming_fields:
            log_data = _load_extraction_log(job_id)
            now = vdata["updated"]
            for field_key, decision in incoming_fields.items():
        ...
        print(f"  Warning: Could not save combined verify for {job_id}: {e}")
    finally:
        conn.close()
...
def save_verification(job_id):
    ...
    # ── FAST PATH (T-UX-CONFIRM-FASTPATH): minimal synchronous writes ──
    # Single connection + transaction for both writes to minimize lock time
    incoming_fields = payload.get("fields") or {}
    _save_verify_combined(job_id, data, incoming_fields)
...
:root {
  ...
  /* Extended palette for theme-aware elements */
  --hover-bg: #FAFAF8;
  --focus-bg: #EBF5FB;
  --confirmed-bg: #F0FBF4;
  --corrected-bg: #FFF8E8;
...
.btn-secondary:hover { background: var(--bg-card); border-color: var(--navy-light); }
...
.form-input { width: 100%; padding: 8px 12px; border: 1px solid var(--border); border-radius: 6px; font-size: 13px; font-family: var(--sans); transition: var(--transition); background: var(--input-bg); color: var(--text); }
...
table.data-table tbody tr:hover { background: var(--hover-bg); }
...
.pill { display: inline-flex; align-items: center; gap: 4px; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; cursor: pointer; border: 1px solid var(--border); background: var(--pill-bg); transition: var(--transition); }
...
/* ═══ UPLOAD SECTION ═══ */
.upload-area { border: 2px dashed var(--border); border-radius: var(--radius-lg); padding: 48px 24px; text-align: center; cursor: pointer; transition: var(--transition); background: var(--upload-bg); }
.upload-area:hover, .upload-area.dragover { border-color: var(--accent); background: var(--upload-hover-bg); }
...
.console-output { background: var(--console-bg); color: var(--console-text); font-family: var(--mono); font-size: 11px; padding: 12px; border-radius: 6px; max-height: 320px; overflow-y: auto; margin-top: 12px; line-height: 1.6; }
.console-output .line-highlight { color: var(--console-hl); }
...
.review-pdf { background: var(--pdf-bg); overflow: auto; display: flex; align-items: flex-start; justify-content: center; padding: 16px; }
...
.field-entity { font-size: 12px; color: var(--text-secondary); padding: 6px 14px; background: var(--entity-bg); border-bottom: 1px solid var(--border-light); display: flex; align-items: center; justify-content: space-between; }
...
.field-row.focused { background: var(--focus-bg); }
.field-row.vf-confirmed { background: var(--confirmed-bg); }
.field-row.vf-corrected { background: var(--corrected-bg); }
.field-row.vf-flagged { background: var(--flagged-bg); }
...
/* Verify buttons */
.vf-btn { width: 26px; height: 26px; border-radius: 5px; border: 1px solid var(--border); background: var(--btn-bg); cursor: pointer; font-size: 13px; display: flex; align-items: center; justify-content: center; transition: var(--transition); color: var(--text-light); }
...
/* Category dropdown */
.cat-select { font-size: 11px; padding: 2px 4px; border: 1px solid var(--border); border-radius: 4px; background: var(--btn-bg); max-width: 150px; cursor: pointer; }
...
.kbd-card { background: var(--bg-card); border-radius: var(--radius-lg); padding: 24px; max-width: 400px; box-shadow: var(--shadow-lg); }
...
/* ═══ MULTI-FILE UPLOAD ═══ */
.file-row { display:flex; align-items:center; gap:8px; padding:8px 12px; border-bottom:1px solid var(--border-light); font-size:13px; }
.file-row:last-child { border-bottom:none; }
.file-row .file-icon { color:var(--accent); flex-shrink:0; }
.file-row .file-name { flex:1; font-weight:500; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.file-row .file-size { color:var(--text-light); font-size:11px; flex-shrink:0; }
.file-row .file-remove { cursor:pointer; color:var(--text-light); font-size:16px; line-height:1; padding:2px 4px; border-radius:4px; border:none; background:none; }
.file-row .file-remove:hover { color:var(--red); background:var(--red-bg); }
.file-row-settings { display:flex; gap:6px; align-items:center; flex-shrink:0; }
.file-row-settings select, .file-row-settings input { font-size:11px; padding:2px 6px; border:1px solid var(--border); border-radius:4px; background:var(--input-bg); color:var(--text); }
.file-row-settings select { max-width:120px; }
.file-row-settings input[type="number"] { width:60px; }
/* Multi-job processing */
.multi-job-list { display:flex; flex-direction:column; gap:6px; margin-bottom:16px; }
.multi-job-row { display:flex; align-items:center; gap:10px; padding:8px 12px; background:var(--bg-card); border:1px solid var(--border-light); border-radius:var(--radius); cursor:pointer; transition:var(--transition); }
.multi-job-row:hover { border-color:var(--accent); }
.multi-job-row.active { border-color:var(--accent); box-shadow:0 0 0 2px rgba(52,152,219,0.15); }
.multi-job-row .mj-name { flex:1; font-size:13px; font-weight:500; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.multi-job-row .mj-bar { width:120px; height:4px; background:var(--border); border-radius:2px; overflow:hidden; flex-shrink:0; }
.multi-job-row .mj-bar-fill { height:100%; background:var(--accent); transition:width 0.3s; }
.multi-job-row .mj-status { font-size:11px; font-weight:600; min-width:60px; text-align:right; }
...
.modal-content { background:var(--bg-card); border-radius:12px; padding:24px; width:420px; max-width:90vw; box-shadow:0 20px 60px rgba(0,0,0,0.3); }
...
.guided-evidence { background: var(--pdf-bg); overflow: auto; display: flex; align-items: flex-start; justify-content: center; padding: 16px; }
...
<aside class="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a class="nav-item" onclick="showSection('upload')" data-section="upload">
...
<div class="section" id="sec-upload">
  <div class="page-header"><h2>Upload Documents</h2><p>Drop one or more PDFs to extract structured data</p></div>
  <div class="page-content">
    <div class="card">
      <div class="card-body">
        <div class="upload-area" id="dropZone" onclick="document.getElementById('fileInput').click()">
          ...
          <p>Select multiple files at once — supports tax documents, bank statements, invoices, checks, and more</p>
        </div>
        <input type="file" id="fileInput" accept=".pdf" multiple style="display:none" onchange="handleFiles(this)">

        <div class="upload-form" id="uploadForm">
          <!-- Mode toggle -->
          <div style="display:flex;gap:8px;margin-bottom:16px">
            <div class="pill active" id="modeSingle" onclick="setUploadMode('single')">Single Job (merge PDFs)</div>
            <div class="pill" id="modeSeparate" onclick="setUploadMode('separate')">Separate Jobs</div>
            <button class="btn btn-ghost btn-sm" onclick="resetUpload()" style="margin-left:auto">Clear All</button>
          </div>

          <!-- File list -->
          <div id="fileList" style="max-height:200px;overflow-y:auto;margin-bottom:16px;border:1px solid var(--border);border-radius:var(--radius);"></div>
          ...
          <details style="margin-top:8px">
            <summary style="cursor:pointer;font-size:13px;color:var(--text-muted);user-select:none">&#x2699; AI Instructions</summary>
            <div style="margin-top:8px">
              <div class="form-group">
                <textarea id="aiInstructions" class="form-input" rows="3" placeholder="Tell the AI how to handle this document. Example: 'This is a trust return — extract K-1 box 1-14 only'"></textarea>
              </div>
            </div>
          </details>
...
<div class="section" id="sec-processing">
  ...
  <div class="page-content">
    <!-- Multi-job tracker (hidden for single jobs) -->
    <div id="multiJobTracker" style="display:none; margin-bottom:16px;">
      <div class="card"><div class="card-body">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
          <span style="font-size:13px;font-weight:600" id="multiJobSummary">0 of 0 complete</span>
        </div>
        <div class="multi-job-list" id="multiJobList"></div>
      </div></div>
    </div>
...
function loadDashboard() {
  fetch('/api/dashboard').then(function(r){ return r.json(); }).then(function(data) {
    ...
    _renderDashPipeline(data.pipeline);
...
function _animateCounter(el, target, prefix, suffix, decimals) {
  ...
  var duration = 800;
  var startTime = null;
  var card = el.closest('.dash-kpi-card');
  ...
  function step(ts) {
    ...
    if (progress < 1) {
      ...
      if (card) setTimeout(function() { card.classList.remove('counting'); }, 400);
    }
  }
  requestAnimationFrame(step);
}

function _dashFormatTime(secs) {
...
function _renderDashCharts(charts) {
  ...
  var commonOpts = {
    ...
    scales: {
      x: { ticks: { color: c.textSec, font: { size: 10 } }, grid: { color: c.border + '44', drawBorder: false } },
      y: { ticks: { color: c.textSec, font: { size: 10 } }, grid: { color: c.border + '44', drawBorder: false }, beginAtZero: true },
    },
  };

  // 1. Job Volume Bar
  var labels = charts.daily_series.map(function(d) { return d.label; });
  _dashCharts.volume = new Chart(document.getElementById('chartJobVolume'), {
    type: 'bar',
    data: { labels: labels, datasets: [{ data: charts.daily_series.map(function(d) { return d.jobs; }),
      backgroundColor: c.accent + '99', borderColor: c.accent, borderWidth: 1, borderRadius: 4, barPercentage: 0.7 }] },
    options: JSON.parse(JSON.stringify(commonOpts)),
  });

  // 2. Document Types Doughnut
  var dtLabels = Object.keys(charts.doc_types);
  var dtData = Object.values(charts.doc_types);
  var dtColors = [c.accent, c.green, c.yellow, c.red, c.purple, c.textSec];
  while (dtColors.length < dtLabels.length) dtColors.push(c.border);
  var dtFriendly = {'tax_returns':'Tax Returns','bank_statements':'Bank Statements','trust_documents':'Trust Docs','bookkeeping':'Bookkeeping','payroll':'Payroll','other':'Other'};
  _dashCharts.docTypes = new Chart(document.getElementById('chartDocTypes'), {
    type: 'doughnut',
    data: { labels: dtLabels.map(function(l) { return dtFriendly[l] || l; }),
      datasets: [{ data: dtData, backgroundColor: dtColors.slice(0, dtLabels.length), borderColor: c.bgCard, borderWidth: 2 }] },
    options: { responsive: true, maintainAspectRatio: false, cutout: '65%',
      plugins: { legend: { display: true, position: 'right',
        labels: { color: c.textSec, font: { size: 11 }, padding: 8, usePointStyle: true, pointStyleWidth: 8 } } } },
  });

  // 3. Extraction Quality Horizontal Bar
  var conf = charts.confidence;
  _dashCharts.quality = new Chart(document.getElementById('chartQuality'), {
    type: 'bar',
    data: { labels: ['High Confidence', 'Low Confidence', 'Needs Review'],
      datasets: [{ data: [conf.high, conf.low, conf.needs_review],
        backgroundColor: [c.green + 'CC', c.yellow + 'CC', c.red + 'CC'],
        borderColor: [c.green, c.yellow, c.red], borderWidth: 1, borderRadius: 4, barPercentage: 0.6 }] },
    options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false } },
      scales: {
        x: { ticks: { color: c.textSec, font: { size: 10 } }, grid: { color: c.border + '44', drawBorder: false }, beginAtZero: true },
        y: { ticks: { color: c.textSec, font: { size: 11 } }, grid: { display: false } },
      } },
  });

  // 4. Cost Trend Line
  _dashCharts.cost = new Chart(document.getElementById('chartCostTrend'), {
    type: 'line',
    data: { labels: labels, datasets: [{ data: charts.daily_series.map(function(d) { return d.cost; }),
      borderColor: c.red, backgroundColor: c.red + '1A', fill: true, tension: 0.4,
      pointRadius: 3, pointBackgroundColor: c.red, pointBorderColor: c.bgCard, pointBorderWidth: 2, borderWidth: 2 }] },
    options: { responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false } },
      scales: {
        x: { ticks: { color: c.textSec, font: { size: 10 } }, grid: { color: c.border + '44', drawBorder: false } },
        y: { ticks: { color: c.textSec, font: { size: 10 }, callback: function(v) { return '$' + v.toFixed(2); } },
          grid: { color: c.border + '44', drawBorder: false }, beginAtZero: true },
      } },
  });
}

function _renderDashPipeline(pipeline) {
  var ids = {'draft':'pipeDraft','preparer_review':'pipePreparer','reviewer_review':'pipeReviewer','partner_review':'pipePartner','final':'pipeFinal'};
  Object.keys(ids).forEach(function(stage) {
    var el = document.getElementById(ids[stage]);
    if (el) _animateCounter(el, pipeline[stage] || 0, '', '', 0);
  });
}
...
function _renderDashActivity(items) {
  ...
  var statusBadge = function(s) {
    var cls = {'complete':'badge-green','running':'badge-blue','error':'badge-red','failed':'badge-red','queued':'badge-gray'}[s] || 'badge-gray';
...
function _renderDashTopClients(clients) {
  var el = document.getElementById('dashTopClients');
  if (!clients || !clients.length) {
    el.innerHTML = '<div style="padding:24px;text-align:center;color:var(--text-light)">No client data yet</div>';
    return;
  }
  ...
  el.innerHTML = clients.map(function(c, i) {
    ...
    return '<div class="dash-client-row">' +
      ...
      '<span class="dash-client-count">' + c.count + '</span></div>';
  }).join('');
}

function _renderDashHealth(health) {
  document.getElementById('healthVersion').textContent = 'v' + health.version;
  document.getElementById('healthUptime').textContent = health.uptime_h + 'h';
  document.getElementById('healthDisk').textContent = health.disk_free_gb + ' GB';
  document.getElementById('healthErrorRate').textContent = health.error_rate_7d + '%';
  document.getElementById('healthTotalJobs').textContent = health.total_jobs;
}

// Load dashboard on initial page load (default section)
...
let uploadedFiles = [];  // Array of {file, client, year, docType}
let uploadMode = 'single';  // 'single' or 'separate'

function handleFiles(input) { if (input.files.length) addFiles(input.files); }

function addFiles(fileList) {
  for (let i = 0; i < fileList.length; i++) {
    const f = fileList[i];
    if (!f.name.toLowerCase().endsWith('.pdf')) { showToast(f.name + ' is not a PDF — skipped', 'error'); continue; }
    if (uploadedFiles.some(u => u.file.name === f.name && u.file.size === f.size)) continue; // skip dupes
    uploadedFiles.push({ file: f, client: '', year: '', docType: '' });
  }
  if (uploadedFiles.length === 0) return;
  document.getElementById('uploadForm').classList.add('visible');
  dropZone.style.display = 'none';
  // Auto-match client from first file
  if (!document.getElementById('clientName').value) {
    const stem = uploadedFiles[0].file.name.replace(/\.pdf$/i, '').replace(/[_-]/g, ' ').toLowerCase();
    const sel = document.getElementById('clientName');
    for (let i = 0; i < sel.options.length; i++) {
      if (sel.options[i].value && stem.includes(sel.options[i].value.toLowerCase())) { sel.value = sel.options[i].value; break; }
    }
  }
  renderFileList();
  updateStartBtn();
...
function removeFile(idx) {
  ...
  if (uploadedFiles.length === 0) { resetUpload(); return; }
  renderFileList();
  updateStartBtn();
}
...
function renderFileList() {
  ...
  const clientOpts = Array.from(clientSel.options).map(o => o.value).filter(Boolean);
  let html = '';
  uploadedFiles.forEach((uf, idx) => {
    html += '<div class="file-row">';
    html += '<span class="file-icon">&#x1F4C4;</span>';
    html += '<span class="file-name" title="' + esc(uf.file.name) + '">' + esc(uf.file.name) + '</span>';
    html += '<span class="file-size">' + formatSize(uf.file.size) + '</span>';
    if (isSeparate) {
      html += '<div class="file-row-settings">';
      html += '<select onchange="uploadedFiles[' + idx + '].client=this.value"><option value="">Global</option>';
      clientOpts.forEach(c => { html += '<option value="' + esc(c) + '"' + (uf.client === c ? ' selected' : '') + '>' + esc(c) + '</option>'; });
      html += '</select>';
      html += '<input type="number" value="' + (uf.year || '') + '" placeholder="Year" min="2000" max="2030" onchange="uploadedFiles[' + idx + '].year=this.value">';
      html += '<select onchange="uploadedFiles[' + idx + '].docType=this.value"><option value="">Global</option>';
      DOC_TYPES.forEach(dt => { html += '<option value="' + dt.id + '"' + (uf.docType === dt.id ? ' selected' : '') + '>' + dt.label + '</option>'; });
      html += '</select>';
      html += '</div>';
    }
    html += '<button class="file-remove" onclick="removeFile(' + idx + ')" title="Remove">&times;</button>';
    html += '</div>';
  });
  el.innerHTML = html;
}
...
function updateStartBtn() {
  ...
  if (uploadedFiles.length <= 1) {
    ...
    btn.textContent = 'Start Extraction (' + uploadedFiles.length + ' PDFs merged)';
  } else {
    btn.textContent = 'Start All (' + uploadedFiles.length + ' jobs)';
  }
}
...
function startExtraction() {
  ...
  if (uploadMode === 'single' || uploadedFiles.length === 1) {
    // Single job — send all PDFs to merge endpoint (or single to existing)
    const fd = new FormData();
    uploadedFiles.forEach(uf => fd.append('pdf', uf.file));
    fd.append('mode', uploadedFiles.length > 1 ? 'single' : 'single');
    fd.append('year', document.getElementById('taxYear').value);
    fd.append('client_name', cn);
    fd.append('doc_type', selectedDocType);
    fd.append('output_format', 'tax_review');
    ...
    fetch('/api/upload-multi', { method: 'POST', body: fd })
      .then(r => r.json())
      .then(data => {
        document.getElementById('startBtn').disabled = false;
        if (data.error) { showToast(data.error, 'error'); return; }
        currentJobId = data.job_id || (data.job_ids && data.job_ids[0]);
        multiJobIds = data.job_ids || [data.job_id];
        document.getElementById('processingFile').textContent = uploadedFiles.length > 1
          ? uploadedFiles.length + ' PDFs merged' : uploadedFiles[0].file.name;
        resetUpload();
        showSection('processing');
        if (multiJobIds.length > 1) startMultiPolling(); else startPolling();
      })
      .catch(e => { showToast('Upload failed: ' + e, 'error'); document.getElementById('startBtn').disabled = false; });
  } else {
    // Separate jobs — upload each file sequentially
    const globalYear = document.getElementById('taxYear').value;
    const globalDocType = selectedDocType;
    const sharedOpts = {
      output_format: 'tax_review',
      user_notes: document.getElementById('userNotes').value,
      ai_instructions: document.getElementById('aiInstructions').value,
      skip_verify: document.getElementById('skipVerify').checked ? 'true' : 'false',
      disable_pii: document.getElementById('disablePii').checked ? 'true' : 'false',
      use_ocr_first: document.getElementById('useOcrFirst').checked ? 'true' : 'false',
    };
    multiJobIds = [];
    multiJobNames = [];
    ...
    uploadedFiles.forEach((uf, idx) => {
      chain = chain.then(() => {
        const fd = new FormData();
        fd.append('pdf', uf.file);
        fd.append('year', uf.year || globalYear);
        fd.append('client_name', uf.client || cn);
        fd.append('doc_type', uf.docType || globalDocType);
        Object.keys(sharedOpts).forEach(k => fd.append(k, sharedOpts[k]));
        return fetch('/api/upload', { method: 'POST', body: fd })
          .then(r => r.json())
          .then(d => {
            if (d.error) { showToast(uf.file.name + ': ' + d.error, 'error'); }
            else { multiJobIds.push(d.job_id); multiJobNames.push(uf.file.name); }
          });
      });
    });
    chain.then(() => {
      document.getElementById('startBtn').disabled = false;
      if (multiJobIds.length === 0) { showToast('No jobs started', 'error'); return; }
      currentJobId = multiJobIds[0];
      document.getElementById('processingFile').textContent = multiJobIds.length + ' jobs queued';
      resetUpload();
      showSection('processing');
      if (multiJobIds.length > 1) startMultiPolling(); else startPolling();
    });
...
function startMultiPolling() {
  if (multiPollTimer) clearInterval(multiPollTimer);
  document.getElementById('multiJobTracker').style.display = '';
  document.getElementById('procCancelBtn').style.display = '';
  // Set the first job as active for the detail view
  currentJobId = multiJobIds[0];
  renderMultiJobList();
  multiPollTimer = setInterval(pollMultiStatus, 1000);
  pollMultiStatus();
}

function renderMultiJobList() {
  const el = document.getElementById('multiJobList');
  let html = '';
  multiJobIds.forEach((jid, idx) => {
    const name = (multiJobNames && multiJobNames[idx]) || jid;
    const isActive = jid === currentJobId;
    html += '<div class="multi-job-row' + (isActive ? ' active' : '') + '" onclick="selectMultiJob(\'' + jid + '\')" data-jid="' + jid + '">';
    html += '<span class="mj-name">' + esc(name) + '</span>';
    html += '<div class="mj-bar"><div class="mj-bar-fill" id="mjbar-' + jid + '" style="width:0%"></div></div>';
    html += '<span class="mj-status" id="mjst-' + jid + '">queued</span>';
    html += '</div>';
  });
  el.innerHTML = html;
}

function selectMultiJob(jid) {
  currentJobId = jid;
  document.querySelectorAll('.multi-job-row').forEach(r => r.classList.toggle('active', r.dataset.jid === jid));
  // Immediately poll this job's details
  pollStatus();
}

function pollMultiStatus() {
  let doneCount = 0;
  let totalCount = multiJobIds.length;
  multiJobIds.forEach(jid => {
    fetch('/api/status/' + jid).then(r => r.json()).then(data => {
      const bar = document.getElementById('mjbar-' + jid);
      const st = document.getElementById('mjst-' + jid);
      if (bar) bar.style.width = (data.progress || 0) + '%';
      if (st) {
        st.textContent = data.status || 'queued';
        st.style.color = data.status === 'complete' ? 'var(--green)' : (data.status === 'failed' ? 'var(--red)' : '');
      }
      if (data.status === 'complete' || data.status === 'failed' || data.status === 'interrupted') doneCount++;
      // Update summary
      document.getElementById('multiJobSummary').textContent = doneCount + ' of ' + totalCount + ' complete';
      // If all done, stop multi-polling
      if (doneCount === totalCount) {
        clearInterval(multiPollTimer); multiPollTimer = null;
        document.getElementById('procCancelBtn').style.display = 'none';
        showToast('All ' + totalCount + ' jobs complete!', 'success');
      }
    }).catch(() => {});
  });
  // Also poll the currently selected job for detail view
  pollStatus();
}
